...
def manage():
    ...
    # Generate list of Rick Rolls to display
    # (but don't bother if the user is not signed in)
    if is_signed_in:
        rick_rolls = [rick_roll for rick_roll in database.rick_rolls
            if not rick_roll.verified]
        rick_rolls = rick_rolls[-config.RICK_ROLLS_DISPLAYED:]
        rick_rolls.reverse()
    else:
        rick_rolls = []
